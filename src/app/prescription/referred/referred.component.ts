...
export class ReferredComponent implements OnInit, AfterViewInit, OnChanges {

  // Public properties
  @Input() referredVisits: CustomVisitModel[] = [];
  @Input() referredVisitsCount: number = 0;
  @Output() fetchPageEvent = new EventEmitter<{page: number, pageSize: number, searchTerm?: string}>();

  // ViewChild references
  @ViewChild('referredPaginator') paginator: MatPaginator;
  @ViewChild('refSearchInput', { static: true }) searchElement: ElementRef;

  // Component state
  displayedColumns: string[] = ['name', 'age', 'visit_created', 'location', 'cheif_complaint', 'status', 'referred_to'];
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.referredVisits && !changes.referredVisits.firstChange) {
      this.updatePaginationState();
      this.updateDataSource();
      this.updatePaginator();
    }
  ...
  private updatePaginationState(): void {
    this.recordsFetched = this.referredVisits.length;
  }

  private updateDataSource(): void {
    const config = this.paginationService.createPaginationConfig(
      this.pageSize,
      this.referredVisitsCount,
      this.recordsFetched
    );
  ...
  private updatePaginator(): void {
    if (this.paginator) {
      this.paginator.length = this.referredVisitsCount;
