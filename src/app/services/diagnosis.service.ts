...
export class DiagnosisService {
  ...
  getObs(patientId, conceptId): Observable<any> {
    ...
    const url = `${this.baseURL}/obs?patient=${patientId}&v=custom:(uuid,value,comment,dateCreated,obsDatetime,creator:(uuid,person:(display)),encounter:(visit:(uuid),encounterProviders:(display,uuid)))&concept=${conceptId}`;
    return this.http.get(url);
  }
