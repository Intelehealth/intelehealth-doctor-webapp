...
export class VisitEncounterPatientOrderService {
  ...
  deleteOrder(uuid: string): Observable<any> {
    const url = `${this.baseURL}/order/${uuid}?purge=true`;
  ...
  getExternalAppointmentList(mpiID: string, visitId: string): Observable<any> {
    const url = `${this.baseURL}/external-appointment/findAppointmentByPatientAndVisitId/${mpiID}/${visitId}`;
    return this.http.get(url);
  }
  ...
  getMiscellaneousRequest(path: "MedicationRequest" | "ServiceRequest" | "Observation", mpiId: string): Observable<any> {
    const url = `${this.baseURL}/shr/bundle/${path}?mpiId=${mpiId}`;
    return this.http.get(url);
  }
}
