...
export class VisitService {
  ...
  getVisits(): Observable<any> {
    ...
    const url = `${this.baseURL}/visit?includeInactive=true&v=custom:(uuid,patient:(uuid,identifiers:(identifier),person:(display,gender,age,birthdate)),location:(display),encounters:(display,encounterDatetime,voided,encounterType:(display),encounterProviders),stopDatetime,attributes)`;
  ...
  patientInfo(id): Observable<any> {
    // tslint:disable-next-line: max-line-length
    const url = `${this.baseURL}/patient/${id}?v=custom:(identifiers,person:(display,gender,age,birthdate,preferredAddress:(cityVillage,postalCode,stateProvince),attributes:(value,attributeType:(display))))`;
    return this.http.get(url);
  }
