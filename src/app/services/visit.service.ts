...
export class VisitService {
  ...
  getVisits(): Observable<any> {
    ...
    const url = `${this.baseURL}/visit?includeInactive=true&v=custom:(uuid,patient:(uuid,identifiers:(identifier),person:(display,gender,age,birthdate,preferredAddress:(stateProvince)),attributes),location:(display),encounters:(display,obs:(display,uuid,value),encounterDatetime,voided,encounterType:(display),encounterProviders),stopDatetime,attributes:(attributeType,display,value,uuid,dateCreated,dateChanged))`;
