...
//         this.response = response.results[0].uuid;
//         this.uuid = this.response
//         console.log(' this.response: ',  this.response);
        
//         // this.response[0].results.uuid;
//         if (result.length !== 0 ) {
...
//   yesNoVal(value){
//     this.msg = [];
//     const msgVal = value == 1 ? 'Yes': 'No'
//    if(value==1){
//     this.msg.push({ uuid: this.uuid, value: msgVal });
//    }else{
//     this.msg.push({ uuid: this.uuid, value: msgVal });
//    }
//   }
  
//   openMsgBox(valueInput){
//     if(valueInput==1){
//       this.showMsgBox = 1
//      }else{
//       this.showMsgBox = 0
  ...
  ngOnInit() {
    ...
    this.visitService.fetchVisitDetails(visitId)
      ...
      .subscribe(response => {
        const result = response.results;
        if (result.length !== 0 ) {
          // this.getSpeciality = result[0].value; 
          this.msg.push({ uuid: result[0].uuid, value: result[0].value });
          this.interaction.value.interaction = 'Yes'
        }
      });
  }
