          ...
          this.service.vitals(vitalUUID).subscribe((vitals) => {
            ...
            vital.forEach((obs) => {
              ...
              if (displayObs.match("BLOOD_GLUCOSE_AFTER_FOOD") !== null) {
                this.dignosis.glucoseAf = Number(
                  obs.display.slice(26, obs.display.length)
                );
              }
              if (displayObs.match("Blood Glucose") !== null) {
                this.dignosis.glucoseF = Number(
                  obs.display.slice(26, obs.display.length)
                );
              }
              if (displayObs.match("Haemoglobin") !== null) {
                this.dignosis.hgb = Number(
                  obs.display.slice(23, obs.display.length)
                );
              }
              if (displayObs.match("Uric Acid") !== null) {
                this.dignosis.uricAcid = Number(
                  obs.display.slice(11, obs.display.length)
                );
              }
              if (displayObs.match("Total Chlolestrol") !== null) {
                this.dignosis.totalChlolestrol = Number(
                  obs.display.slice(19, obs.display.length)
                );
              }
              if (displayObs.match("BLOOD_GLUCOSE_RANDOM_ID") !== null) {
                this.dignosis.glucoseRandom = Number(
                  obs.display.slice(24, obs.display.length)
                );
              }
              if (displayObs.match("BLOOD_GLUCOSE_POST_PRANDIAL_ID") !== null) {
                this.dignosis.glucosePrandial = Number(
                  obs.display.slice(31, obs.display.length)
                );
              }
            });
