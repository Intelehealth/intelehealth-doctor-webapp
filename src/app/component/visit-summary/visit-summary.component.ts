    ...
    this.visitService.patientInfo(this.patientId).subscribe((info) => {
      ...
      let patientNo = info.person.attributes.find(
        ...
        this.visitService.shortUrl(link).subscribe((res: { data }) => {
          ...
          let smsText: string = `Swami Vivekananda Helpline Project Dear ${patientInfo.name} Your prescription is available to download at ${shortLink} - Powered by Intelehealth`;
