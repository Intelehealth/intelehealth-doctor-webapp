  ...
  ngOnInit() {
    ...
    this.service.getVisits().subscribe(
      (response) => {
        ...
        var visits1 = userRoles.length > 0 ? pVisits : pVisits.filter(a => a.attributes.length > 0 ? (a.attributes.find(b => b.value == this.specialization)) : "")

       if (this.specialization && this.specialization.toLowerCase() == "all") {
        visits1 = response.results;
      } else {
        visits1 = response.results.filter((a) =>
