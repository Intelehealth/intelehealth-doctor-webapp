...
export class HwProfileComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.personalInfoForm = new FormGroup({
      ...
      countryCode1: new FormControl(this.isNepalClient ? '+977' : '+91'),
      countryCode2: new FormControl(this.isNepalClient ? '+977' : '+91'),
      ...
      // } else {
        this.authService.getProvider(JSON.parse(localStorage.user).uuid).subscribe((provider: any) => {
          if (provider.results.length) {
            ...
            this.hwName = provider.results[0].person.display;
